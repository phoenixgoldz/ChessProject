...
namespace SharpChess.Model.AI
{
    ...
    public static class OpeningBookSimple
    {
        ...
        public static void Initialise()
        {
            ...
            Console.WriteLine("[OpeningBookSimple] Initializing...");

            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                xmlDoc.Load("OpeningBook.xml"); // 🔹 Make sure this file exists
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR] OpeningBook.xml failed to load: {ex.Message}");
                return; // Prevent crash if file is missing
            }

            Console.WriteLine("[OpeningBookSimple] XML Loaded Successfully.");

            XmlElement xmlnodeRoot = xmlDoc.DocumentElement;
            if (xmlnodeRoot == null)
            {
                Console.WriteLine("[ERROR] XML Root is NULL. Skipping Opening Book Initialization.");
                return;
            }

            BuildHashtable(xmlnodeRoot, Game.PlayerWhite);
            BuildHashtable(xmlnodeRoot, Game.PlayerBlack);
        }
        ...
        private static void BuildHashtable(XmlElement xmlnodeParent, Player player)
        {
            ...
            {
                Console.WriteLine("[ERROR] xmlnodeParent is NULL in BuildHashtable(). Skipping initialization.");
                return; // Prevent the crash
            }
            if (player == null)
            {
                throw new Exception("[ERROR] player is NULL in BuildHashtable()!");
            }

            Console.WriteLine($"[OpeningBookSimple] Hashtable Building Started for {player.Colour}...");
