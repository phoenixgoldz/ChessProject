...
using System.Linq;
using System;
using MoveNames = SharpChess.Model.Move.MoveNames;
    ...
    public static class Game
    {
        ...
        static Game()
        {
            try
            {
                ...
                Console.WriteLine("Initializing HashTables...");
                ...
                Console.WriteLine("PlayerWhite initialized.");
                PlayerBlack = new PlayerBlack();
                Console.WriteLine("PlayerBlack initialized.");

                PlayerToPlay = PlayerWhite;
                Console.WriteLine("Setting Board Hash Key...");
                Board.EstablishHashKey();

                Console.WriteLine("Initializing Opening Book...");

                PlayerWhite.Brain.ReadyToMakeMoveEvent += PlayerReadyToMakeMove;
                PlayerBlack.Brain.ReadyToMakeMoveEvent += PlayerReadyToMakeMove;

                Console.WriteLine("Finalizing game initialization...");
                OpeningBookSimple.Initialise(); // 🔹 Only call this after board setup is 100% done

            }
            catch (Exception ex)
            {
                Console.WriteLine($"Game Initialization Failed: {ex.Message}");
        ...
        public static ChessMode CurrentMode { get; set; } = ChessMode.Traditional;
        ...
        private static void SetupChess960Board()
        {
            Console.WriteLine("[DEBUG] Setting up Chess960 board...");

            // Call Chess960 setup methods in PlayerWhite & PlayerBlack
            PlayerWhite.SetPiecesAtStartingPositions();
            PlayerBlack.SetPiecesAtStartingPositions();

            // Debug print the randomized board
            Console.WriteLine("[DEBUG] Chess960 board setup complete.");
        }
        ...
        ///   Start a new game.
        /// </summary>
        public static void New(ChessMode mode)
        {
            CurrentMode = mode;
            Console.WriteLine($"[DEBUG] Starting new game: {mode}");

            if (mode == ChessMode.Chess960)
            {
                SetupChess960Board();
            }
            else
        ...
        {
            New(ChessMode.Traditional);
            Console.WriteLine($"[DEBUG] New game started in mode: {CurrentMode}");

            if (CurrentMode == ChessMode.Chess960)
            {
                SetupChess960Board();
            }
            else
            {
                NewInternal(); // Traditional chess setup
            }
        }
        ...
        public static void StartNormalGame()
        {
            try
            {
                Console.WriteLine("[DEBUG] Starting normal game...");
                Game.New();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR] StartNormalGame failed: {ex.Message}");
            }
            PlayerToPlay.Clock.Start();
            ResumePondering();
        }
        ...
        private static bool LoadGame(string strFileName)
        {
            try
            {
                Console.WriteLine($"[DEBUG] Loading game from: {strFileName}");
                // Existing game loading code...
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR] LoadGame failed: {ex.Message}");
                return false;
            }
            ...
            if (xmlnodelist != null)
            {
                ...
                {
                    Square from = Board.GetSquare(xmlnode.GetAttribute("From"));
                    Square to = Board.GetSquare(xmlnode.GetAttribute("To"));

                    if (from.Piece == null)
                    {
                        Console.WriteLine($"[ERROR] LoadGame: Move '{xmlnode.GetAttribute("Name")}' has a NULL piece! From: {from}, To: {to}");
                    }
                }
                ...
                foreach (XmlElement xmlnode in xmlnodelist)
                {
                    ...
                    // ✅ Fix: Ensure the piece is not null before making the move
                    if (from.Piece == null)
                    {
                        Console.WriteLine($"[ERROR] LoadGame: Move '{xmlnode.GetAttribute("Name")}' has a NULL piece! Square: {from}");
                        continue; // Skip this move instead of crashing
                    }
        ...
        /// <param name="piece"> The piece to move. </param>
        /// <param name="square"> The square to move to. </param>
        private static void MakeAMoveInternal(MoveNames moveName, Piece piece, Square square)
        {
            Console.WriteLine($"[DEBUG] MakeAMoveInternal called with move: {moveName}, piece: {(piece != null ? piece.Name.ToString() : "NULL")}, square: {square}");

            if (piece == null)
            {
                Console.WriteLine($"[ERROR] MakeAMoveInternal: piece is NULL! Move: {moveName}, Square: {square}");
                throw new Exception("[ERROR] MakeAMoveInternal: piece is NULL!");
            }
            if (square == null)
            {
                Console.WriteLine($"[ERROR] MakeAMoveInternal: square is NULL!");
                throw new Exception("[ERROR] MakeAMoveInternal: square is NULL!");
            }
